...
function ensureEditPin() {
  if (editUnlocked) return true;
  const pin = prompt("PIN de operador (solo quien confirma entregas):");
  if (pin === CONFIG.EDIT_PIN) {
    editUnlocked = true;
    sessionStorage.setItem("obsequios_edit_ok", "1");
    return true;
  }
  if (pin !== null) showToast("PIN incorrecto", "err");
  return false;
}
...
async function registrarEnvio() {
  if (!selectedKey || selectedRowStatus !== "SEPARADO - FACTURADO" || !ensureEditPin()) return;
  ...
  if (
    !confirm(
      `¿Registrar envío del obsequio a CLIENTE para:\n${nombre}\n\nObsequio: ${obsequios}\n\nSe descontará del inventario.`
    )
  ) {
    return;
  }
  ...
  try {
    ...
    showToast(`Envío registrado: ENVIADO A CLIENTE`, "ok");
    switchTab(2);
  } catch (e) {
...
async function eliminarEnvio() {
  if (!selectedKey || !ensureEditPin()) return;
  const client = sync.clients[selectedKey];
  ...
  try {
    ...
    showToast(`${nombre} revertido a ${destino}`, "ok");
...
async function confirmarCierre() {
  ...
  const nombre = client.NOMBRE_CLIENTE || selectedKey;
  if (!confirm(`¿Confirmar que ${nombre} recibió el obsequio?\nPasará a Cerrados (sin descontar inventario).`)) return;
  ...
  try {
    ...
    showToast("Entrega confirmada y cerrada", "ok");
    switchTab(3);
  } catch (e) {
    showToast(`Error: ${e.message}`, "err");
  } finally {
...
async function revertirCierre() {
  ...
  const nombre = client.NOMBRE_CLIENTE || selectedKey;
  if (!confirm(`¿Eliminar el cierre de ${nombre}?\nVolverá a la pestaña Enviados.`)) return;
  ...
  try {
    ...
    await sync.patchCliente(selectedKey, payload);
    showToast("Cliente revertido a Enviados", "ok");
    switchTab(2);
  } catch (e) {
    showToast(`Error: ${e.message}`, "err");
  } finally {
...
function init() {
  if (sessionStorage.getItem("obsequios_edit_ok") === "1") editUnlocked = true;
